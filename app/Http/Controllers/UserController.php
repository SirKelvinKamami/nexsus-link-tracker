...
class UserController extends Controller
{
    ...
    public function analytics()
    {
        ...
        $dailyMax = max(1, $daily ? max($daily) : 1);

        $topLinks = Link::where('user_id', $userId)
            ->leftJoinSub(
                LinkClick::where('user_id', $userId)
                    ->selectRaw('link_id, COUNT(*) as clicks')
                    ->groupBy('link_id'),
                'clicks', 'links.id', '=', 'clicks.link_id'
            )
            ->orderByDesc('clicks.clicks')
            ->whereNotNull('link')
            ->where('link', '<>', '')
            ->take(10)
            ->get(['links.id', 'links.title', 'links.link', 'links.type', 'clicks.clicks']);
        ...
        $topMediums = (clone $clicks)
            ->selectRaw('utm_medium, COUNT(*) as total')
        ...
        return view('studio.analytics', compact(
            ...
            'daily', 'dailyMax', 'topLinks', 'topReferrers', 'topSources',
