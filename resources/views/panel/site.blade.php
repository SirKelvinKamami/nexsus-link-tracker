                        ...
                        <h2 class="mb-4 card-header"><i class="bi bi-person"> {{__('messages.Site Customization')}}</i></h2>
                          ...
                          @csrf
                            <div class="form-group col-lg-8">
                              <label>{{__('messages.Site logo')}}</label>@if(file_exists(base_path("assets/Nexsus Tracker/images/").findFile('avatar')))<a title="Remove icon" class="hvr-grow p-1 text-danger" style="padding-left:5px;" href="{{ route('delAvatar') }}"><i class="bi bi-trash-fill"></i></a>@endif
                              <div class="mb-3">
                                <input type="file" class="form-control form-control-lg" name="image" aria-label="Large file input example">
                            </div>
                            </div>
                            <div class="form-group col-lg-8">
                              <label>{{__('messages.Favicon')}}</label>@if(file_exists(base_path("assets/Nexsus Tracker/images/").findFile('favicon')))<a title="Remove icon" class="hvr-grow p-1 text-danger" style="padding-left:5px;" href="{{ route('delFavicon') }}"><i class="bi bi-trash-fill"></i></a>@endif
                              <div class="mb-3">
